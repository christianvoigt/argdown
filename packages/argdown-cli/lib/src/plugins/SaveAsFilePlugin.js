...
class SaveAsFilePlugin {
    ...
    async runAsync(request, response, logger) {
        ...
        let fileContent = !this.settings.isRequestData ? response[this.settings.dataKey] : request[this.settings.dataKey];
        if (!_.isEmpty(fileContent) && _.isString(fileContent)) {
            ...
            const dataSettings = !this.settings.isRequestData ? request[this.settings.dataKey] : null;
            if (dataSettings && dataSettings.outputDir) {
                outputDir = dataSettings.outputDir;
            }
            await this.saveAsFile(fileContent, outputDir, fileName, this.settings.extension, logger);
        }
    }
    getFileName(file) {
