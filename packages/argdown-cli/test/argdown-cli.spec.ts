...
const execPromise = (cmd: string, callback: IExecCallback) => {
  return new Promise((resolve, reject) => {
    require("child_process").exec(cmd, (error: Error | null, stdout: any, stderr: any) => {
      if (error !== null) {
        reject(error);
      }
...
describe("argdown-cli", function() {
  ...
  it("can include files", () => {
    ...
    return execPromise(cmd, (error, stdout, stderr) => {
      ...
      expect(stderr).to.equal("");
      expect(stdout).to.not.equal(null);
      expect(stdout.replace(/File \'.+\' already included./g, "File already included.")).to.equal(
        expectedResult.replace(/File \'.+\' already included./g, "File already included.")
      );
    });
  });
