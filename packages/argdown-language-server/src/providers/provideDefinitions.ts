...
export const provideDefinitions = (response: IArgdownResponse, uri: string, position: Position): Location[] => {
  ...
  if (nodeAtPosition && isTokenNode(nodeAtPosition)) {
    ...
    if (tokenName.startsWith("Statement")) {
      ...
      return findReferences(response, nodeAtPosition, true).map(n => createLocation(uri, n));
    }
  }
  return [];
};
