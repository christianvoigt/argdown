...
connection.onInitialized(() => {
    connection.console.log("Argdown language server initialized. Node version: " + process.version);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (result.parserErrors && result.parserErrors.length > 0) {
            for (var error of result.parserErrors) {
                ...
                var range = vscode_languageserver_1.Range.create(start, end);
                var message = error.message;
                var severity = vscode_languageserver_1.DiagnosticSeverity.Error;
                var diagnostic = vscode_languageserver_1.Diagnostic.create(range, message, severity, "argdown");
...
connection.onRenameRequest((params) => __awaiter(this, void 0, void 0, function* () {
    ...
    const doc = documents.get(textDocument.uri);
    const response = yield processDocForProviders(doc);
    return index_1.provideRenameWorkspaceEdit(response, newName, position, textDocument);
}));
connection.onHover((params) => __awaiter(this, void 0, void 0, function* () {
    const { textDocument, position } = params;
    const response = yield processDocForProviders(textDocument);
    return index_1.provideHover(response, position);
}));
...
connection.onCompletion((params) => __awaiter(this, void 0, void 0, function* () {
    ...
    const response = yield processTextForProviders(input, path);
    return index_1.provideCompletion(response, char, position, txt, offset);
}));
connection.onDocumentHighlight((params) => __awaiter(this, void 0, void 0, function* () {
    const { textDocument, position } = params;
    const response = yield processDocForProviders(textDocument);
    return index_1.provideReferences(response, textDocument.uri, position).map((l) => vscode_languageserver_1.DocumentHighlight.create(l.range, 1));
}));
connection.onReferences((params) => __awaiter(this, void 0, void 0, function* () {
    const { context, position, textDocument } = params;
    const response = yield processDocForProviders(textDocument);
    return index_1.provideReferences(response, textDocument.uri, position, context);
}));
connection.onDefinition((params) => __awaiter(this, void 0, void 0, function* () {
    const { textDocument, position } = params;
    const response = yield processDocForProviders(textDocument);
    return index_1.provideDefinitions(response, textDocument.uri, position);
}));
node_1.argdown.addPlugin(DocumentSymbolsPlugin_1.documentSymbolsPlugin, "add-document-symbols");
connection.onDocumentSymbol((params) => __awaiter(this, void 0, void 0, function* () {
    const path = vscode_uri_1.default.parse(params.textDocument.uri).fsPath;
    ...
    const response = yield node_1.argdown.runAsync(request);
...
connection.onExecuteCommand((params) => __awaiter(this, void 0, void 0, function* () {
    if (params.command === EXPORT_CONTENT_COMMAND) {
        ...
        yield Export_1.exportContent(node_1.argdown, args);
    }
        ...
        yield Export_1.exportDocument(node_1.argdown, args, doc);
    }
        ...
        for (var workspaceFolder of workspaceFolders) {
            let settings = yield getDocumentSettings(workspaceFolder.uri);
            let rootPath = vscode_uri_1.default.parse(workspaceFolder.uri).fsPath;
            if (!settings.configFile || settings.configFile === "") {
                return;
            }
            let configPath = path.resolve(rootPath, settings.configFile);
            let config = yield node_1.argdown.loadConfig(configPath);
            if (config !== null) {
                if (!config.rootPath) {
                    config.rootPath = rootPath;
                }
                yield node_1.argdown.load(config);
            }
        }
    }
}));
