...
function getSvgString(el: SVGSVGElement, width: number, height: number, scale: number, css: string = "") {
  ...
  source = source.replace(
    /height="100%"/,
    'height="' + height * scale + 'px" viewBox="0 0 ' + width + " " + height + '" preserveAspectRatio="xMinYMin meet"'
  );
...
function svgString2Image(svgString: string, width: number, height: number, callback: (pngString: string) => void) {
  //   var imgsrc =
...
export function getPngAsString(el: SVGSVGElement, scale: number, css: string, callback: (pngString: string) => void) {
  var width = el.clientWidth;
  var height = el.clientHeight;
  if (width === 0 || height === 0) {
    var box = el.getBoundingClientRect();
