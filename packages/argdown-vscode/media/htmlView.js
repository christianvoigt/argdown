...
const state_1 = __webpack_require__(/*! ./state */ "./preview/state.ts");
var scrollDisabled = true;
const marker = new activeLineMarker_1.ActiveLineMarker();
const settings = settings_1.getSettings();
const vscode = acquireVsCodeApi();
vscode.postMessage({});
const store = new state_1.ArgdownPreviewStore(vscode);
...
document.addEventListener("dblclick", event => {
    ...
    const line = scroll_sync_1.getEditorLineNumberForPageOffset(offset, store.getState().html.lineCount || 0);
...
if (settings.scrollEditorWithPreview) {
    window.addEventListener("scroll", throttle(() => {
        if (scrollDisabled) {
            ...
            const line = scroll_sync_1.getEditorLineNumberForPageOffset(window.scrollY, store.getState().html.lineCount || 0);
            if (typeof line === "number" && !isNaN(line)) {
...
function getEditorLineNumberForPageOffset(offset, lineCount) {
    const { previous, next } = getLineElementsAtPageOffset(offset);
    if (previous) {
        const previousBounds = previous.element.getBoundingClientRect();
        const offsetFromPrevious = offset - window.scrollY - previousBounds.top;
        if (next) {
            const progressBetweenElements = offsetFromPrevious /
...
class ArgdownPreviewStore {
    constructor(vscode) {
        ...
        if (window.initialState &&
            ...
            this._state = defaultsDeep({}, serializedState, window.initialState, defaultState);
        }
